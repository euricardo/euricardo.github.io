...
var imgarray, hrefarray, title = [];
var script;

window.onload = function(){
	if(jQuery("#pagination_gall").length  > 0){
		imgarray = jQuery(".venobox-gallery-obj div img").map(function() {
			return jQuery(this).attr("src");
		});
		hrefarray = jQuery(".venobox-gallery-obj div a").map(function() {
			return jQuery(this).attr("href");
		});
		title = jQuery(".venobox-gallery-obj div img").map(function() {
...
function paginate_gallery(x){
	
	var columns = jQuery("#pagination_gall").eq(0).data('cols');
	var imgSize = jQuery("#pagination_gall").eq(0).data('list-size');
	var thumbmargin = jQuery("#pagination_gall").eq(0).data('thumbmargin');
	var thumbbgcolor = jQuery("#pagination_gall").eq(0).data('thumbbgcolor');
	var thumbborder = jQuery("#pagination_gall").eq(0).data('thumbborder');
	var Html, k, spanClass, size, l, script_tag,
	i = 0;
	var total_paginate = hrefarray.length / imgSize;
	...
	for(k = 0 ; k < imgarray.length; k++) {
		...
		Html += '><a href="'+hrefarray[k]+'" data-gall="gallery" class="venobox" target="_self" title="'+title[k]+'"><img src="'+imgarray[k]+'" title="'+title[k]+'" alt="'+title[k]+'" class="img-responsive venoframe" style="border: '+thumbborder+'px solid '+thumbbgcolor+' !important; margin: '+thumbmargin+'px;"></a></div>';
		if ( (i % columns == columns - 1) && (i != imgarray.length -1) ) {
			Html += '</div>';
			Html += '<div class="sm-row-fluid">';
		}
		i++;	
	...
	var pagin, end; 
	pagin = '<div class="pagination" id="pagination_gall" data-list-size="'+imgSize+'" data-thumbborder="'+thumbborder+'" data-thumbbgcolor="'+thumbbgcolor+'" data-thumbmargin="'+thumbmargin+'" data-cols="'+columns+'">'
	pagin += '<li href="#" class="prev"';
	if(x > 1){
		pagin +=  'onclick="paginate_gallery('+(x - 1)+');"';
	}
	pagin += '>&laquo;</li>';
	for(i = 0; i < total_paginate; i++){
		...
		pagin += ' page-numbers" onclick="paginate_gallery('+(i + 1)+');">'+(i + 1)+'</li>';
	}
	...
	jQuery(".venobox-gallery-obj div").remove();
	jQuery(".venobox-gallery-obj").append(Html);
	jQuery(".venobox-gallery-obj").append(script_tag);
	jQuery(".venobox-gallery-obj").append(pagin);
}
